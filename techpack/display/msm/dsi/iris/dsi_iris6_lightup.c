...
static struct iris_ctrl_seq *_iris_get_ctrl_seq_common(int32_t type)
{
	...
	if (type == IRIS_CONT_SPLASH_NONE)
		pseq = iris_get_ctrl_seq_addr(pcfg->timing[pcfg->cur_timing].ctrl_seq, pcfg->chip_id);
	else if (type == IRIS_CONT_SPLASH_LK)
		pseq = iris_get_ctrl_seq_addr(pcfg->timing[pcfg->cur_timing].ctrl_seq_cs, pcfg->chip_id);
...
static int32_t _iris_alloc_pip_buf(struct iris_ip_index *pip_index)
{
	...
	for (i = 0; i < ip_cnt; i++) {
		...
		if (opt_cnt != 0) {
			pip_index[i].opt =
				vzalloc(opt_cnt * sizeof(struct iris_ip_opt));
			if (!pip_index[i].opt) {
				/*free already malloc space*/
				for (j = 0; j < i; j++) {
					vfree(pip_index[j].opt);
...
struct iris_ip_opt *iris_find_ip_opt_mask(uint8_t ip, uint8_t opt_id, uint8_t opt_mask)
{
	...
	for (i = 0; i < pip_index->opt_cnt; i++) {
		popt = pip_index->opt + i;
		IRIS_LOGE("opt_cnt: %d, opt_id: 0x%x", pip_index->opt_cnt, popt->opt_id);
...
static int32_t _iris_verify_dtsi(const struct iris_cmd_header *hdr,
		const struct iris_ip_index *pip_index)
{
	uint32_t *pval = NULL;
	uint8_t tmp = 0;
	int32_t rc = 0;
	...
	if (rc) {
		IRIS_LOGE("%s(), hdr info: %#x %#x %#x %#x %#x %#x", __func__,
	...
	for (i = 0; i < data_cnt; i++) {
		...
		while (data_len >= sizeof(struct iris_cmd_header)) {
			...
			if (IRIS_IF_LOGVV()) {
				...
				if (rc) {
					IRIS_LOGE("%s(%d), verify dtis return: %d",
...
int iris_parse_param(struct device_node *np, struct dsi_panel *panel)
{
	...
	if (!strcmp(panel->name,"ASUS ams678 cmd mode dsc fhd plus panel") ||
			!strcmp(panel->name,"ASUS ams678 er2 cmd mode dsc fhd plus panel")) {
		pcfg->is_mp_panel = 1;
		IRIS_LOGI("%s(), It's MP panel\n", __func__);
	} else {
		pcfg->is_mp_panel = 0;
		IRIS_LOGI("%s(), It's DEV panel\n", __func__);
		return 0;
	}
	...
	pcfg->dsirecover_check_path = PATH_I2C;
...
{
	if (!cmds || !cmds->count) {
		IRIS_LOGE("%s(), invalid cmds: %p or cmd cnt is 0", __func__, cmds);
		return;
	}
...
void iris_alloc_update_ipopt_space(void)
{
	...
	p = vmalloc(sum * sizeof(struct iris_update_ipopt));
	if (!p) {
	...
	memset(pcfg->timing[pcfg->cur_timing].pq_update_cmd.update_ipopt_array, 0x00,
			sum * sizeof(struct iris_update_ipopt));
...
void iris_read_power_mode(struct dsi_panel *panel)
{
	...
	if (iris_get_abyp_mode_blocking() == IRIS_ABYP_MODE) {
		iris_dsi_send_cmds(panel, cmdset.cmds, cmdset.count, cmdset.state, pcfg->vc_ctrl.to_panel_hs_vc_id);
	} else {
		iris_dsi_send_cmds(panel, cmdset.cmds, cmdset.count, cmdset.state, pcfg->vc_ctrl.to_panel_hs_vc_id);
		read_cmd_rbuf[0] = 0;
...
static void _iris_send_iris_cmds(struct dsi_panel *panel, int type)
{
	...
	_iris_send_panel_cmd(panel, &panel_cmds);
}
...
int iris_enable(struct dsi_panel *panel, struct dsi_panel_cmd_set *on_cmds)
{
	...
	uint32_t timeus0 = 0;
	uint32_t delay = 0;
#define PANEL_DELAY_US (60*1000)

	IRIS_LOGI("%s(), lightup opt: 0x%x", __func__, lightup_opt);

	iris_lp_enable_pre();

	rc = iris_dsi_send_cmds(panel, on_cmds->cmds,
			on_cmds->count, on_cmds->state, 0);
	...
	if (iris_get_abyp_mode_nonblocking() == IRIS_PT_MODE) {
		...
		while (1) {
			...
			if ((lightup_opt & 0x10) == 0) {
				...
				if (iris_platform_get() == 0) { /* for FPGA */
					rc = iris_abyp_switch_proc(pcfg->display, IRIS_PT_MODE, true);
					if (rc != 0) {
	...
	ktime1 = ktime_get();
	timeus0 = (u32) ktime_to_us(ktime1) - (u32)ktime_to_us(ktime0);
	if (timeus0 < PANEL_DELAY_US) {
		delay = PANEL_DELAY_US - timeus0;
		usleep_range(delay, delay);
	}
	IRIS_LOGI("%s() spend time0 %d us, delay %d us.",
		__func__, timeus0, delay);
		...
		struct dsi_mode_info *mode_info)
{
	...
	if (lightup_opt & 0x8) {
		rc = iris_dsi_send_cmds(panel, switch_cmds->cmds,
				switch_cmds->count, switch_cmds->state, 0);
...
int iris_lightoff(struct dsi_panel *panel, struct dsi_panel_cmd_set *off_cmds)
{
	...
	if (iris_get_abyp_mode_blocking() == IRIS_PT_MODE) {
		...
		iris_dsi_send_cmds(panel, off_cmds->cmds,
			off_cmds->count, off_cmds->state, 0);
		iris_quality_setting_off();
	}
	iris_lp_setting_off();
...
int iris_dsirecover_check(enum iris_op_type op_type)
{
	...
	IRIS_LOGD("dsirecover_check_method = %d", pcfg->dsirecover_check_method);
/*
	if ((op_type == IRIS_LIGHTUP_OP) || (op_type == IRIS_ESDCHECK_OP)) {
...
static ssize_t _iris_dbg_split_pkt_write(struct file *file,
		const char __user *buff, size_t count, loff_t *ppos)
{
	unsigned long val;
...
static ssize_t _iris_dbg_split_pkt_read(struct file *file,
		char __user *buff, size_t count, loff_t *ppos)
{
	uint8_t type;
	int len, tot = 0;
...
static const struct file_operations iris_split_pkt_fops = {
	.open = simple_open,
	.write = _iris_dbg_split_pkt_write,
	.read = _iris_dbg_split_pkt_read,
};
...
static int _iris_dbg_init_cont_splash(struct dsi_display *display)
{
	...
	if (debugfs_create_file("iris_split_pkt", 0644, pcfg->dbg_root, display,
		...
		IRIS_LOGE("%s(%d): failed to create file 'iris_split_pkt'",
			__FILE__, __LINE__);
		return -EFAULT;
	}
...
void iris_deinit(void)
{
	...
	for (pcfg->cur_timing = 0; pcfg->cur_timing < pcfg->timing_cnt; pcfg->cur_timing++) {
		...
		if (pcfg->timing[pcfg->cur_timing].pq_update_cmd.update_ipopt_array) {
			vfree(pcfg->timing[pcfg->cur_timing].pq_update_cmd.update_ipopt_array);
