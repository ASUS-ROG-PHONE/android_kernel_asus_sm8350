...
static uint8_t *__iris_i2c_alloc_buf(uint32_t byte_len)
{
	...
	IRIS_LOGD("%s: buf = %p, index = %d, byte_len = %d", __func__,
	...
	IRIS_LOGD("%s: buf = %p, index = %d", __func__, pbuf, pcfg->iris_i2c_cfg.buf_index);
...
int iris_i2c_read(uint32_t addr, uint32_t *pval, uint32_t reg_num)
{
	...
	ret = i2c_transfer(iris_i2c_handle->adapter, msgs, 2);
	if (ret == 2) {
		ret = 0;
	} else {
		ret = ret < 0 ? ret : -EIO;
		IRIS_LOGE("%s, %d: i2c_transfer failed, addr = 0x%08x, ret = %d\n", __func__, __LINE__, addr, ret);
	}
