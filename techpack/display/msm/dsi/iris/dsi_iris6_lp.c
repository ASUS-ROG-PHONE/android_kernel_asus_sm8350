...
void _iris_abyp_stop(void)
{
	...
	if (debug_on_opt & 0x20) {
		while (1) {
			IRIS_LOGE("ABYP switch stop here! cnt: %d", i++);
			msleep(3000);
			if (!(debug_on_opt & 0x20)) {
...
enter_abyp_begin:
	...
	abyp_status_gpio = _iris_lp_check_gpio_status((frame_interval*2/3 + 1), 1);
...
static void _iris_dphy_itf_check(void)
{
	int ret = 0;
	...
	if (ret) {
		IRIS_LOGE("%s, read dphy_itf_ctrl fail, ret = %d", __func__, ret);
		return;
	}
...
int iris_lp_abyp_exit(bool blocking)
{
	...
	/*get the gpio status in 2-3 frames*/
	abyp_status_gpio = _iris_lp_check_gpio_status((frame_interval*2/3 + 1), 0);
	...
	if (debug_lp_opt & 0x08)  // for error process debug
		abyp_status_gpio = 1;
...
int iris_abyp_switch_proc(struct dsi_display *display, int mode, bool blocking)
{
	...
	u32 regs[] = {0xf000003c, 0xf1100204, 0xf115ffe4};
