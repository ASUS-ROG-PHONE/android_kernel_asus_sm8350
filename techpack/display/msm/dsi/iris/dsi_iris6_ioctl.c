	...
	if (cmd.flags & MSMFB_MIPI_DSI_COMMAND_TO_PANEL) {
		if (iris_get_abyp_mode_blocking() == IRIS_PT_MODE)
			...
			iris_dsi_send_cmds(pcfg->panel, cmdset.cmds,
					cmdset.count, cmdset.state, cmd.vc);
		...
		iris_dsi_send_cmds(pcfg->panel, cmdset.cmds, cmdset.count, cmdset.state, cmd.vc);
	}
...
static int _iris_configure(u32 type, u32 value)
{
	...
	case IRIS_DBG_KERNEL_LOG_LEVEL:
		...
		break;
	case IRIS_OSD_ENABLE:
		if (value == 0x40)  //dport disable
			iris_dport_disable(0x1, 0x1);
		else if (value == 0x41)  //dport enable
	...
	case IRIS_SDR2HDR_AI_INPUT_AMBIENTLIGHT:
		...
		if (pqlt_cur_setting->ai_auto_en == 1)
...
int update_dc_brightness(uint32_t *dimmingvalues, uint32_t dimmingEnable,  uint32_t brightness)
{
	struct iris_cfg *pcfg = iris_get_cfg();
	struct iris_setting_info *iris_setting = iris_get_setting();
	uint32_t bl_lvl;
...
int iris_update_dc_brightness(void)
{
	...
	if (pcfg->dc_bl_pending == 1) {
		IRIS_LOGE("dc_bl_pending = 1 ");
		update_dc_brightness(&pcfg->pend_dimming[0], pcfg->pend_dimmingEnable,  pcfg->pend_brightness);
...
int iris_configure_ex(u32 type, u32 count, u32 *values)
{
	...
	#else
		{
			uint32_t csc_values[5];
			csc_values[0] = values[0];
			csc_values[1] = values[1];
			csc_values[2] = values[2];
			csc_values[3] = values[3];
			csc_values[4] = values[4];
			if (pcfg->panel->bl_config.bl_inverted_dbv) {
				uint32_t bl_lvl = values[3];
			...
			iris_dcDimming_backlight_set(&csc_values[0]);
		}
	#endif
		break;
