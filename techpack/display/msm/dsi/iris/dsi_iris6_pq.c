...
static u32 iris_ai_break_Y[7] = {
	0x01D600D8, 0x05AC0463, 0x0888072B, 0x0C9B0AEB, 0x0FBD0E31, 0x0FDA0FDA, 0x00000FDA,
};
...
void iris_pq_parameter_init(void)
{
	...
	iris_dportdis_debug = true;
	IRIS_LOGE("%s, iris_min_x_value=%d, iris_max_x_value = %d", __func__, iris_min_x_value, iris_max_x_value);
}
...
int iris_dcDimming_backlight_set(u32 *values)
{
	...
	if (bl_lvl > 0xffff) {
		pr_err("invalid backlight params\n");
		return -EINVAL;
	}
...
int iris_cm_ratio_set(uint8_t chain)
{
	...
	if (1 /*pcfg->lut_mode == SINGLE_MODE*/) {
		...
		if ((xvalue >= iris_max_x_value) && (xvalue < xvalue_default)) {
			...
			IRIS_LOGE("ratio:%d, xvalue: %d, iris_max_x_value: %d", ratio, xvalue, iris_max_x_value);
			for (i = 0; i < 18; i++) {
			...
			IRIS_LOGE("ratio:%d, xvalue: %d, iris_min_x_value: %d", ratio, xvalue, iris_min_x_value);
			for (i = 0; i < 18; i++) {
	...
	IRIS_LOGE("cm color temperature value=%d", value);
	return len;
}
...
void iris_cm_colortemp_mode_set(u32 mode, bool bcommit)
{
	...
	IRIS_LOGE("cm color temperature mode=%d, len=%d", mode, len);
}
...
void iris_cm_color_temp_set(void)
{
	...
	/*}*/
	IRIS_LOGE("%s, len = %d",  __func__, len);
}
...
void iris_cm_color_gamut_pre_set(u32 source_switch)
{
	...
	IRIS_LOGE("source switch = %d, len=%d", source_switch, len);
}
...
void iris_dport_disable(bool bdisable, uint8_t chain)
{
	...
	if (iris_dportdis_debug) {
		...
		len = iris_end_dport();
		IRIS_LOGE("dport_disable=%d, len=%d", bdisable, len);
...
void iris_dpp_3dlut_interpolation(u8 enable, u32 interSrc, u32 lutgain, bool bcommit)
{
	...
	len = iris_end_dpp(bcommit);
	IRIS_LOGE("3dlut interpolation, enable=%d, len=%d", enable, len);
}
...
void iris_dpp_3dlut_gain(u32 lutgain, bool bcommit)
{
	...
	len = iris_init_update_ipopt_t(IRIS_IP_DPP, 0x52, 0x52, 0x01);
	len = iris_end_dpp(bcommit);
	IRIS_LOGE("3dlut interpolation, gain=0x%x, len=%d", lutgain, len);
}
...
void iris_cm_color_gamut_set(u32 level, bool bcommit)
{
	...
	len = iris_end_dpp(bcommit);
	IRIS_LOGE("cm color gamut=%d, len=%d", level, len);
}
...
void iris_dpp_demo_window_set(u8 enable, u8 owAndGamma, u32 xWindow, u32 yWindow)
{
	...
	uint32_t  *payload = NULL;
	uint32_t regvalue;
	IRIS_LOGE("iris_dpp_demo_window_set X:0x%x Y:0x%x, enable,out[%d, %d]", xWindow, yWindow, enable, owAndGamma);

	len = iris_start_demo();
	// RESERVED
	payload = iris_get_ipopt_payload_data(IRIS_IP_DPP, 0x63, 4);
	IRIS_LOGE("DPP< 0xE0, pos 4, 0x%x", payload[0]);
	// payload[0]: WND_CTRL
	...
	IRIS_LOGI("iris_dpp_demo_window_set X[0x%x], Y[0x%x], enable: %d len=%d", xWindow, yWindow, enable, len);
...
void iris_dpp_fingerDisplay_set(u8 enable, u8 shapeInFill, u32 radius, u32 position, u32 fillcolor)
{
	...
	struct iris_update_regval regval;
	uint32_t regvalue = 0;
	IRIS_LOGE("iris_finger_display set radius:0x%x position:0x%x, fillcolor:0x%x", radius, position, fillcolor);
	...
	IRIS_LOGE("dpp, 0x62, radius: 0x%x", payload[0]);
	regvalue = radius;
...
void iris_dbc_level_set(u32 level)
{
	...
	IRIS_LOGE("send A/B  %d", iris_dbc_lut_index);
	...
	IRIS_LOGE("dbc level=%d, len=%d", level, len);
}
...
static void iris_hdr_ai_enable_replaceY(void)
{
	...
	payload = iris_get_ipopt_payload_data(IRIS_IP_SDR2HDR, 0x4B, 11);
	for (i = 0; i < 7; i++) {
		payload[i] = iris_ai_break_Y[i];
		iris_set_ipopt_payload_data(IRIS_IP_SDR2HDR, 0x4B, 11+i, payload[i]);
	}
}
...
void iris_sdr2hdr_level_set(u32 level, bool bcommit)
{
	...
	IRIS_LOGE("sdr2hdr level =%d", level);
}
...
void iris_hdr_ai_enable(bool bEn)
{
	...
	payload = iris_get_ipopt_payload_data(IRIS_IP_SDR2HDR_2, 0x00, 2);
	ai_ctrl = (payload[0] & (~0x10000)) | ((bEn == true)?0x00010000:0x00000000);
	IRIS_LOGE("hdr ai enable =%d\n", ai_ctrl);
	iris_set_ipopt_payload_data(IRIS_IP_SDR2HDR_2, 0x00, 2, ai_ctrl);
	len = iris_update_ip_opt(IRIS_IP_SDR2HDR_2, 0x00, 0x01);

	len = iris_end_hdr(true);
	IRIS_LOGE("hdr_ai_en=%d, len=%d\n", bEn, len);
}
...
void iris_hdr_ai_input_al(u32 al_value)
{
	...
	IRIS_LOGE("hdr ai input al =%d\n", al_value);
	iris_set_ipopt_payload_data(IRIS_IP_SDR2HDR_2, 0x10, 2, ai_input_al_value);

	if (pqlt_cur_setting->pq_setting.sdr2hdr != SDR2HDR_CASE5_AI)
		return;
	...
	len = iris_end_hdr(true);
	IRIS_LOGE("hdr_ai_input_al_value=%d, len=%d\n", al_value, len);
}
...
void iris_hdr_ai_input_panel_nits(u32 nits_value)
{
	...
	IRIS_LOGE("hdr ai input panel nits =%d\n", nits_value);
	iris_set_ipopt_payload_data(IRIS_IP_SDR2HDR_2, 0x10, 2, ai_input_nits_value);
	len = iris_update_ip_opt(IRIS_IP_SDR2HDR_2, 0x10, 0x01);

	len = iris_end_hdr(true);
	IRIS_LOGE("hdr_ai_input_panel_nits=%d, len=%d\n", nits_value, len);
}
...
void iris_hdr_ai_ctrl_cam_gain(bool bEn)
{
	...
	len = iris_end_hdr(true);
	IRIS_LOGE("hdr_ai_cam_gain=%d, len=%d\n", bEn, len);
}
...
void iris_hdr_ai_ctrl_graphic_weight(bool bEn)
{
	...
	IRIS_LOGE("hdr ai graphic weight  =%d\n", ai_graphic_weight);
	...
	len = iris_end_hdr(true);
	IRIS_LOGE("hdr_ai_graphic_weight=%d, len=%d\n", bEn, len);
}
...
void iris_hdr_ai_ctrl_s_curve(bool bEn)
{
	...
	len = iris_end_hdr(true);
	IRIS_LOGE("hdr_ai_s_curve=%d, len=%d\n", bEn, len);
}
...
void iris_pwm_freq_set(u32 value)
{
	...
	IRIS_LOGE("%s, blc_pwm freq=%d", __func__, regvalue);
}
...
void iris_pwm_enable_set(bool enable)
{
	...
	iris_update_pq_opt(true);
	IRIS_LOGE("%s, blc_pwm enable=%d", __func__, enable);
}
...
void iris_dbc_bl_user_set(u32 value)
{
	...
	IRIS_LOGE("%s,bl_user value=%d", __func__, regvalue);
}
...
void iris_dbc_led0d_gain_set(u32 value)
{
	...
	IRIS_LOGE("%s,dbc_led0d value=%d", __func__, value);
}
...
void iris_scaler_filter_update(u8 scaler_type, u32 level)
{
	...
	IRIS_LOGE("scaler filter level=%d", level);
}
...
int iris_update_backlight(u32 bl_lvl)
{
	...
	if (iris_setting.quality_cur.ai_auto_en == 1)
...
void iris_sdr2hdr_scurve(u32 value)
{
	...
	iris_end_hdr(true);
	IRIS_LOGE("scurve_level = %d", value);
}
